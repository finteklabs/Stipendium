...
const char * const BITCOIN_CONF_FILENAME = "stipendium.conf";
const char * const BITCOIN_PID_FILENAME = "stipendiumd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "stipendium" is a composite category enabling all Stipendium-related debug output
                if(ptrCategory->count(std::string("stipendium"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Stipendium
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Stipendium
    // Mac: ~/Library/Application Support/Stipendium
    // Unix: ~/.stipendium
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Stipendium";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Stipendium";
#else
    // Unix
    return pathRet / ".stipendium";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty stipendium.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override stipendium.conf
