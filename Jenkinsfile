...
// which are part of the Stipendium repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="stipendium-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /stipendium-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "stipendiumpay-stipendium/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from stipendiumpay-stipendium/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'stipendiumpay-stipendium/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from stipendiumpay-stipendium/develop"
            }
          }

          if (hasCache) {
            sh "cd /stipendium-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /stipendium-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /stipendium-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /stipendium-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /stipendium-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /stipendium-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /stipendium-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
