...
# stipendium_hash
RUN git clone https://github.com/stipendiumpay/stipendium_hash
RUN cd stipendium_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} stipendium
RUN useradd -u ${USER_ID} -g stipendium -s /bin/bash -m -d /stipendium stipendium
...
RUN mkdir /stipendium-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /stipendium-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /stipendium-src
